...
func start() {
	...
	if enableBatchScheduler {
		...
		// Fail fast if the default batch scheduler doesn't exist
		if !slices.Contains(registry.GetRegisteredSchedulerNames(), defaultBatchScheduler) {
			logger.Error(nil, "Failed to find registered scheduler for default batch scheduler",
				"scheduler", defaultBatchScheduler)
			os.Exit(1)
		}
	}
